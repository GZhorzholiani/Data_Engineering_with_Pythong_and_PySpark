...
from pyspark.sql.functions import expr, col, from_json, window, sum
...
grouped = joined \
    ...
    .agg(
    sum("ListPrice").alias("total_sales")
)
...
query = final.writeStream \
    ...
    .outputMode("append") \
    .option("checkpointLocation", "checkPoints/MovieSalesMetrics") \
    .start()
